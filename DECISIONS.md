...
## Duplicate key-down — synthetic events posted AFTER the callback

**Symptom.** Typing an English/informal word in Vietnamese mode doubled the
diacritic key: `task`→`tassk`, `fix`→`fixx`, `google`→`gooogle`, `cài`→`cafi`.
Only keys that carry a Telex tone/mark (which make Keystone emit a Backspace)
doubled; plain keys did not.

**Root cause (found with a live event-tap file log).** The tap RECEIVES the
tone key twice for a single physical press (`srcPid=0`, `autorepeat=0` — a fresh
key-down, not an OS key-repeat), 100% correlated with Keystone emitting a
synthetic **Backspace**. The engine is correct — it faithfully composes whatever
keys arrive (`t,a,s,s,k` → the doubled result). The duplicate is generated by
the OS: when the tap callback posts synthetic key events **while it is
suppressing** the physical key (returning nil), the window server re-delivers
that just-suppressed physical key to the tap.

**Fix.** Do not inject during the callback. `EventTapController.handle` now
schedules the `KeystrokeExecutor` run with `CFRunLoopPerformBlock` on the tap's
own run loop, so the backspaces/text are posted only **after** the callback has
returned and the suppression has settled — no echo. `TapSink` also posts each
event with `CGEvent.post(tap: .cgSessionEventTap)` (CGEventPost) instead of
`tapPostEvent(proxy)`; the existing `selfTag` guard drops our own re-entering
events. Same tap thread, so ordering with the next keystroke is preserved. This
is integration-only (a real CGEvent pair + a target app to observe), so it is
not unit-tested; verified by typing on the live app.
